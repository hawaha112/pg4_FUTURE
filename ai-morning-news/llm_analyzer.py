...
class LLMAnalyzer:
    ...
    def generate_broadcast_script(self, digest: dict, items: list = None,
        ...
        sys_msg = (
            "你是顶级中文科技电台的主播兼撰稿人, 风格沉稳、口语、有观点, 像个看了十年行业的老炮在跟你唠。"
            "你写的稿子将被 TTS 逐字朗读成音频, 听众在通勤路上单耳听、会走神、没有进度条、记不住数字。"
            "所以你为耳朵写、不为眼睛写: 读出来不像人话的句子就不写。"
            "你不是在念新闻列表, 是在做一档有编辑观点的节目——新闻之间必须有'为什么接在一起'的逻辑, 不是逐条罗列。"
        )
        user_msg = (
            ...
            "2. 问候+菜单(约70字): 补班次问候+报日期, 然后报'今天几件事'并逐条点名, "
            "再用一句**具体的主线**把它们串起来(不是'指向同一方向'这种空话, 要说清是哪个方向, "
            "比如'都在说定价权从技术领先转向谁成本更低')。结尾说'先看第一件'。\n"
            "3. 头条深讲(只讲最重磅的2条, 每条约250字): 每条开头喊序号'第一件…''第二件…'。"
            "单条内部走: 一句钩子→发生了什么→半句背景→**机制(为什么这会导致后果, 一环扣一环、不跳步, 深度全在这)**→"
            "影响(谁受益谁受损)+一个可证伪的预测。**条与条之间必须用因果过渡**, 讲清'为什么这条之后接那条'"
            "(比如'英伟达这步把国内逼到墙角, 燧原冲IPO就是这股压力下的产物'), 禁止只用'说完这个再看'这种空连接。\n"
            "4. 快讯串播(总共约250字): 先说'接下来是几条快讯'。**按主题成块**(同一领域的并成一段连着说, 不要随机排); "
            "**同一家公司的多条必须合并成一条讲**(比如'OpenAI今天在开发者生态连下两子: 一是…, 二是…')。每条一两句、节奏加快。\n"
            "5. 今日观点(约180字): 开头说类似'最后聊个观点', 并**回指开场那条主线**('说回开头说的…')。"
            "把判断用口语讲清推理链: 事实→机制(因为A所以B)→结论, **不许跳步直接甩大结论**; "
            "结尾给一个具体、可检验的预测(落到公司/数字/时间)。\n"
            "6. 收尾(约45字): 一句金句式结论收束(强化主线、不放新信息)+提醒文字版在简报页面+按班次道别。\n\n"
            "【口播硬规则】\n"
            "- 为耳朵写: 一句一个意思, 句子尽量短(一口气能读完), 书面长句一律拆成几句短的\n"
            "- 标点即停顿: 全篇只用逗号和句号, 不用分号、冒号、破折号、括号(它们对耳朵没意义)\n"
            "- 口语词: 用'会'别用'将', 用'现在'别用'目前', 多用'你/我们''说白了''换句话说'; 删尽形容词堆砌和套话\n"
            "- 专有名词第一次出现先给身份再给名('做AI编程工具的Cursor''英伟达CEO黄仁勋'), 身份在前\n"
            "- 数字口语化且给量级/相对感: '比上一代快三倍'胜过精确百分比; 一句话最多一个关键数字; "
            "说'第三家'必须先点清前两家是谁; 英文名可保留(GPT、OpenAI)\n"
            "- **数字必须素材里有据**: 素材没写的数字一律不编, 改用'增速很快''翻了两倍多'这类弹性说法\n"
            "- **钩子不许夸大事实(最重要的红线)**: 开场和过渡可以选角度、有张力, 但抛出的冲突必须是素材里"
            "真实发生的, 绝不能为了抓人编一个更劲爆的版本。例: 素材是'某公司主动限制自家模型', 就不能说成"
            "'政府强制下线'; 钩子的张力来自'怎么讲', 不是'夸大成什么'。事实一律以素材为准\n"
            "- 详略分三层: 头条深讲、快讯略讲、最次要的并进一句话, 不要每条平均用力(那样像念清单)\n"
            "- 不要 markdown、阿拉伯数字序号、小标题、括号注释、emoji —— 输出将逐字朗读\n"
            "- 段落之间空一行(朗读时自然停顿)\n"
            "- 不确定的事说'据报道', 不要把传闻当事实\n\n"
            f"【素材】\n{material}\n\n"
            f"只输出口播稿正文。再次强调: 总字数 {lo}-{hi} 字, 全篇是一档有主线、有路标、有因果的节目, 不是新闻列表。"
        )
